<template>
  ...
  export default {
    ...
    mounted() {
      ...
      this.$axios
        .get(`/api/categoria/${id_categoria}`)
        .then((response) => {
    ...
    guardarProductosSeleccionados() {
      this.$axios.get('/sanctum/csrf-cookie').then(response => {
        this.$axios.post('/api/productos/guardar-productos-en-session', { productos: this.productosSeleccionados })
          .then(response => {
    ...
    obtenerPrecioTotal() {
      this.$axios.get('/sanctum/csrf-cookie').then(response => {
        this.$axios.post('/api/productos/precioTotal') 
          .then(response => {
            console.log(response.data);
          })
          .catch(error => {
